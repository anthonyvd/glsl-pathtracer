...
float rfloat() {
	return ( RADV(global_lcg), fract(sin(float(global_lcg.next))) );
}
...
vec3 rand_vec_polar(float theta, float phi, float r) {
	return vec3(r) * vec3(sin(phi) * cos(theta), sin(phi) * sin(theta), cos(phi));
}

vec3 rand_in_unit_sphere() {
	return rand_vec_polar( rfloat(0.0, 2.0 * 3.1415), acos((2 * rfloat()) - 1.0), pow(rfloat(), 1.0/3.0) );
}

vec3 bad_rand_in_unit_sphere() {
	vec3 v = rand_vec(-1, 1);
	while (dot(v, v) >= 1) v = rand_vec(-1, 1);
	return v;
}

vec3 rand_unit_vector() {
	return normalize(bad_rand_in_unit_sphere());
}
...
scatter_t scatter_lambertian(ray_t r, hit_t h) {
	vec3 scatter_direction = h.normal + rand_unit_vector();
	...
	return scatter_t(true, h.material.l.albedo, vec3(0), scattered_ray);
}
...
scatter_t scatter_metal(ray_t r, hit_t h) {
	...
	ray_t scattered = ray_t(h.pos + h.sdf_reflected_offset, normalize(reflected + h.material.m.fuzz * bad_rand_in_unit_sphere()));
...
const sphere_t kSpheres[] = sphere_t[](
	// Lambertian spheres
	//sphere_t(vec3( 2,     0,   0), 1.0, MAKE_LAMB(vec3(0.7, 0.3, 0.3)) ),
	sphere_t(vec3( 0, -100.5,   -2), 100, MAKE_LAMB(vec3(0.8, 0.8,   0)) )
	// Metal spheres
	//sphere_t(vec3(-1,      0,   -2), 0.5, MAKE_METAL(vec3(0.8, 0.8, 0.8), 0.3)),
	//sphere_t(vec3( 1,      0,   -2), 0.5, MAKE_METAL(vec3(0.8, 0.6, 0.2), 1.0)),
	// Dielectric spheres
	// sphere_t(vec3(0, 0, 0), 1, MAKE_DIEL(1.5))
	// Emitter spheres
	//sphere_t(vec3(0, 5, 0), 2, MAKE_EMIT(vec3(2)))
...
float scene_sdf(vec3 p) {
	...
	return sub_sdf(box_sdf(p, vec3(0.5, 0.5, 0.5)), octahedron_bound_sdf(p, 1));
...
hit_t eval_sdf(ray_t r, float t) {
	...
	if (abs(dist) < kEpsilon) {
		...
		return hit_t(true, pos /*+ kEpsilon * normal*/, normal, t, dist > 0, /*MAKE_DIEL(1.5)  MAKE_METAL(vec3(0.8, 0.8, 0.8), 0.3) */ MAKE_LAMB(vec3(0.7, 0.3, 0.3)) , kEpsilon * normal, -(2.0 * kEpsilon) * normal);
	}
...
void main() {
	global_lcg = init_lcg(10 ^ floatBitsToInt(gl_FragCoord.x * gl_FragCoord.y));
	...
	vec3 col = vec3(sqrt(color_accumulator.x / samples_per_round), sqrt(color_accumulator.y / samples_per_round), sqrt(color_accumulator.z / samples_per_round));
	color = vec4(col, 1.0);
}